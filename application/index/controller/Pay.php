...
class Pay extends  BaseController
{
    ...
    public function pay()
    {
        ...
        for($i=0;$i<count($sidarr);$i++){
            $stock=Standard::where(['id'=>$sidarr[$i]])->lock(true)->value('stock');
            if($stock<$countarr[$i]){
                throw new Product([
                    'msg'=>'库存量不足,请重新下单',
        ...
        $config = array(
            ...
            //异步通知地址
            'notify_url' => "http://www.tiancloth.com/index/pay/notify",

            //同步跳转
            'return_url' => "http://www.tiancloth.com/index/pay/returnfy",
...
//        OrderModel::where(['order_no'=>$out_trade_no])->update(['status'=>1]);
            ...
            try{
                ...
                for($i=0;$i<count($sidarr);$i++){
                    ...
                    ProductModel::where(['id'=>$product_id])->update(['sell'=>$psell]);
                }
    ...
    public function returnfyMethod()
    {
        ...
        try{
            ...
            for($i=0;$i<count($sidarr);$i++){
                ...
                $product_id=Standard::where(['id'=>$sidarr[$i]])->value('product_id');
                $psell=ProductModel::where(['id'=>$product_id])->value('sell');
                ProductModel::where(['id'=>$product_id])->update(['sell'=>($psell+$countarr[$i])]);
            }
...
//        //同步跳转地址
//        var_dump($post);
        echo "<h1 align='center' style='color: green;margin-top: 200px;'>支付成功<a href='http://www.tiancloth.com/index/order'>返回订单首页</a></h1>";
        exit();
    }
}