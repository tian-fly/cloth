...
//       return $order;
    ...
    public  function order(){
        ...
        for($i=0;$i<count($car_standardIds_arr);$i++){
            $stock=Standard::where(['id'=>$car_standardIds_arr[$i]])->lock(true)->value('stock');
            if($stock<$car_gnums_arr[$i]){
                throw new Product([
                    'msg'=>'库存量不足,请重新下单',
   ...
   //获取购物车商品
    private function highToCar(){
        ...
        for($i=0;$i<count($car_gids_arr)-1;$i++){
            ...
            $products[$i]['standard']='颜色：'.$stanards[$i]['color'].'/尺寸：'.$stanards[$i]['size'];
            $products[$i]['price']=$products[$i]['price'];
            $products[$i]['stock']=$stanards[$i]['stock'];
            $products[$i]['total_one_price']=$products[$i]['price']*$products[$i]['num'];
        }
        return $products;
    }
...
//        $order->products()->attach(1,['count'=>2]);
    ...
    public function send(){
        ...
        if($status!=3){
            return [
                'status'=>0,
                'message'=>'订单未交易成功，请收到货后再评论'
            ];
        }
        $uid=$this->getUid();
        $comment=Comment::where(['user_id'=>$uid,'product_id'=>$data['id'], 'standard_id'=>$data['standard_id']])->find();
        if($comment){
            return [
                'status'=>0,
                'message'=>'每个用户只可评论一次'
        ...
        $user=User::get($uid);
        $result=$user->comments()->save(['content'=>$data['content'],'standard_id'=>$data['standard_id'],'product_id'=>$data['id'],'count'=>$data['count'],'star'=>$data['star']]);
        if($result){
            return [
                'status'=>1,
                'message'=>'评论成功'
